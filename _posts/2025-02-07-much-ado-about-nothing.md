...
The same idea extends to more complex, familiar systems. Consumer operating systems runs many tasks at the same time, sharing the hardware resources among them. Each task takes turns to run for a short period, then it is suspended to give other tasks a chance. *Schedulers* orchestrate this time-sharing behavior at a high speed, so in a user's view, all tasks run simultaneously. A task may *block*, waiting for some non-CPU resources to be available. It may also voluntarily *sleep* for a period of time. Under these circumstances, the scheduler happily assign the processor resources to other *ready* tasks, only waking up the blocked task after its requested resources become ready.

Still, a scheduler could run out of ready tasks. The simple approach is to add a special "idle" task that does few to no jobs. In Linux, [an always-ready `idle` task](https://www.kernel.org/doc/html/v5.0/admin-guide/pm/cpuidle.html#the-idle-loop) is assigned when no other ready tasks are available. Each logical CPU has its own `idle` task. This task puts the running CPU into *idle state* with parameters calculated by sophisticated *governors*. The CPU's idle state is a hardware state, not an OS abstraction, so it differs among each architecture and CPU model and will be covered [later in this post](#sleep-electronically).

Windows uses a different set of terminology[^1] to describe its ~~scheduling~~ *dispatching* behavior. Each CPU has an *idle thread*[^2], which runs when the CPU is idle. All idle threads belong to an *idle process*, the one you see as "System Idle Process" in Task Manager.
...
I guess the first line of each pair is what most of us would write.
...
Some other architectures have special registers that can be utilized to perform a nop. One example of this is the `x0` register in RISC-V RV32/64, whose ABI name is `zero`. It is hardwired to unsigned 0, and all writes to it are ignored[^5]. This feature leads to an intuitive implementation of nop: writing `x0` to `x0`. In RV32 assembly, it writes as:
...
Why it is **not** considered a nop, even its destination is `zero`?
...
Modern processors tries to make maximum use of its hardware components. Instead of running instructions one by one, waiting each of them to finish, they use many techniques to run statistically many instructions in one cycle. Typical techniques include:

* *Pipelining.* Just like making products in a factory, an instruction's execution can be divided into many *stages*. It has to be fetched from memory, decoded, result calculated, then persisted into memory or registers. When the previous instruction finishes decoding, the next instructions don't have to wait until it finishes. Instead, the next one can start decoding immediately.
* *Reordering.* Some instructions may have dependencies among each other. For example, one instruction's source operand is the preceding one's destination. It can not be calculated before the result of preceding instruction is persisted. Modern processors select unrelated instructions to the pipeline to minimize these hazards.
* *Multiple issuing.* A hardware core may have more than one arithmetic logic unit (ALUs) that does the actual calculation. In this case, many arithmetic instructions may run simultaneously.
* *Speculation.* Instead of waiting a branch's result to be calculated, high-performance processor may guess its jump destination, and start to run instructions there. If the prediction is correct, much time can be saved. Contrarily, if predicted incorrectly, all wrong instructions must be discarded.
* ... And many more.

However, these implementation details may differentiate some instructions that have no differences on ISA level.

Consider a simple pipelined RV32 processor having these stages: Fetching, Decoding, Execution, Memory Access, Register Update. These following two sequences behave the same on ISA level, but the latter one would be significantly slower if the processor is implemented poorly:
...
Here, Sequence 2 contains a branch never taken since `zero` always equals `zero`. However, the processor will predict it as taken since it jumps backwards. It will have started running instructions at `-0x40` before it calculates that the branch is not taken. It will then have to discard all instructions fetched from `-0x40`, and start all over again at the correct place.

From these examples, we can see that even both you and your coworker are doing nothing, the price you pay for slacking off is still probably different.

## Sleep electronically

Many factors may affect the processor's power consumption. Frequency, active silicon components, and (rarely) output signal levels are some common factors. Almost all sorts of processors and controllers have different *power states*, since it's a common practice to put the chip to sleep when it's idle. Generally speaking, when a processor or controller enters a low-power state or "sleep state", most of its peripherals are switched off (or "stopping the clock" in engineers' jargons). Its execution is paused, and can only be woken up by interrupts. In consumer embedded applications, the user interaction is usually infrequent, so putting the device to sleep can greatly extend its battery life. Your TV remote is probably sleeping for the majority of its lifespan.
...
* C0 state is the running state. The processor runs normally, and it's clock is optionally throttled by ACPI-defined mechanism.
...
The power consumption decreases as power state transits from C0 to Cn, while the entry/exit latency increases.
